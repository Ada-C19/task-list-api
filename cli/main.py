...
def get_task_from_user(msg="Input the id of the task you would like to work with: "):
    ...
    if not tasks:
        app.print_stars(
            "This option is not possible because there are no tasks.")
        return task
    count = 0
...
def create_task():
    ...
    response = app.create_task(title, description)
    print_task(response)


def view_task():
    task = get_task_from_user(
        "Input the id of the task you would like to select ")
    if task:
        print("\nSelected Task:")
        print_task(task)
...
def edit_task():
    ...
    if task:
        ...
        response = app.update_task(task["id"], title, description)
        print("\nUpdated Task:")
        print_task(response)
...
def delete_task_ui():
    task = get_task_from_user(
        "Input the id of the task you would like to delete: ")
    if task:
...
def change_task_complete_status(status):
    ...
    if task:
        if status:
            response = app.mark_complete(task["id"])
        else:
            response = app.mark_incomplete(task["id"])
